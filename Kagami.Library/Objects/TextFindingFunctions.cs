...
public static class TextFindingFunctions
{
   ...
   {
      return new(input.Split([value], StringSplitOptions.None).Select(String.StringObject).ToArray());
   }
   ...
   public static Tuple partition(string value, string input, bool reverse)
   {
      ...
      else
      {
         ...
         {
            return Tuple.Tuple3(input.Keep(_index), value, input.Drop(index + value.Length));
         }
         else
         {
            return Tuple.Tuple3(input, "", "");
