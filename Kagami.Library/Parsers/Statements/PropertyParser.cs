...
public partial class PropertyParser : StatementParser
{
      ...
      Maybe<TypeConstraint> _typeConstraint)
   {
      ...
      else
      {
         ...
         parameters = new Parameters(new Parameter(false, "", "value", nil, _typeConstraint, false, false));
      }
   ...
   public override Optional<Unit> ParseStatement(ParseState state, Token[] tokens)
   {
      ...
      if (_block is (true, var block))
      {
         ...
         state.AddStatement(new Function(propertyName, parameters, block, yielding, isOverride, ""));
         return unit;
      }
      else
      {
