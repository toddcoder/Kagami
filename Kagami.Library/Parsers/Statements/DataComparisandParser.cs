...
namespace Kagami.Library.Parsers.Statements
{
   public class DataComparisandParser : StatementParser
   {
      ...
      public override IMatched<Unit> ParseStatement(ParseState state, Token[] tokens)
      {
         ...
         if (result.If(out var item, out var original))
         {
            (var comparisands, var newOrdinal) = item;
      ...
      static IMatched<IObject[]> getPossibleComparisands(bool hasArguments, string className, string name, ParseState state)
      {
         ...
         if (hasArguments)
         {
            ...
            if (result.If(out var comparisands, out var original))
      ...
      static IMatched<IRangeItem> getOrdinal(ParseState state, IRangeItem ordinal)
      {
         if (state.Scan("^ /(|s|) /'='", Color.Whitespace, Color.Structure).If(out _, out var isNotMatched, out var exception))
            if (getValue(state, ExpressionFlags.Standard).If(out var value, out var original))
