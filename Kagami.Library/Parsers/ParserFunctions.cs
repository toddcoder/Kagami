...
public static class ParserFunctions
{
   ...
   public static StringSet keywords =
      ["do", "else", "true", "false", "return", "if", "print", "println", "printline", "put", "undef", "and", "or", "in", "each"];
   ...
   public static Optional<PossibleTypeConstraint> parseCollectionTypeConstraint(ParseState state, TypeTailEnd tailEnd = TypeTailEnd.None)
   {
      ...
      if (_begin is (true, var begin))
      {
         ...
         if (isSet)
         {
            ...
            if (_leftPossibleTypeConstraint is (true, { Maybe: (true, var leftTypeConstraint) }))
            {
               if (state.Scan("^(:)", Color.Class))
               {
                  ...
                  {
                     var subTypeConstraint = leftTypeConstraint.Merge(rightTypeConstraint);
                     if (tailEnd is TypeTailEnd.DictionaryOrSet)
                     {
                        return new PossibleTypeConstraint.Some(subTypeConstraint);
                     }
                     else
                     {
               ...
               else if (state.Scan("^(})", Color.Class))
               {
                  if (tailEnd is TypeTailEnd.DictionaryOrSet)
                  {
                     return new PossibleTypeConstraint.Some(leftTypeConstraint);
                  }
                  else
                  {
         ...
         else if (isArray)
         {
            ...
            {
               if (tailEnd is TypeTailEnd.Array)
               {
                  return new PossibleTypeConstraint.Some(arrayTypeConstraint);
               }
               else
               {
   ...
   public static Optional<TaggedExpression[]> getTaggedExpressions(ParseState state)
   {
      ...
      while (state.More)
      {
         ...
         else if (_tag.Exception is (true, var exception))
         {
            return exception;
         }
         else
