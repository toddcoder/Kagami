...
public partial class Playground : Form
{
   ...
   protected void Playground_Load(object sender, EventArgs e)
   {
      ...
      try
      {
         ...
         menus.Menu("println", (_, _) => insertText("println ", 0, 0), "^P");
         menus.Menu("print", (_, _) => insertText("print ", 0, 0));
   ...
   protected void update(bool execute, bool fromMenu, bool ignoreDirtyFlag, bool selectedText)
   {
      ...
      if (!locked && textEditor.TextLength != 0)
      {
         ...
         try
         {
            ...
            var parameters = new ExecutionParameters(context, kagamiConfiguration, source, execute, packageFolder.FullPath, traceFile);
            var background = new ExecuteBackground(parameters)
            {
               TraceOutput =
               {
                  Handler = t => { traceFile.TryTo.SetText(t, Encoding.UTF8); }
               },
               OperationStarting =
               {
                  Handler = tuple => { }
            ...
            background.Finalized.Handler = () =>
               {
                  stopwatch.Stop();
                  var state = textEditor.StopAutoScrollingAlways();
                  cancelled = background.Cancelled;
                  var type = background.Type;
                  var compiler = background.Compiler.Value;
                  var value = background.Value;
                  _exceptionIndex = background.ExceptionIndex;
                  status = background.Status;
                  var machine = background.Machine.Value;
                  try
                  {
