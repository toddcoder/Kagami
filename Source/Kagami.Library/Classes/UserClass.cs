...
public class UserClass : BaseClass
{
   ...
   public UserClass(string className, string parentClassName)
   {
      ...
      _parentClass = (maybe<BaseClass>() & this.parentClassName.IsNotEmpty() & (() => Module.Global.Class(parentClassName))).Map(bc => (UserClass)bc);
   }
   ...
   {
      return _metaObject.Map(uo => classOf(uo).RespondsTo(selector)) | false;
   }
